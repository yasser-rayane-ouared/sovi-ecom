                        ...
                        <ol className="list-decimal list-inside space-y-1">
                          {language === 'ar' ? (
                            <>
                              ...
                              <li>اكتب تسمية للموقع، واختر نوع <b>reCAPTCHA v3</b>، وأضف نطاق متجرك (مثل: <code>{selectedStore?.custom_domain || `${selectedStore?.subdomain}.${getRootDomain()}`}</code> أو <code>{selectedStore?.subdomain}.{getRootDomain()}</code>) <b>دون كتابة <code>https://</code> في البداية ودون كتابة أي مسار أو شرطة في النهاية مثل <code>/mc</code> (يجب كتابة النطاق الصافي فقط)</b>.</li>
                            ...
                            <>
                              ...
                              <li>Enter a label, select <b>reCAPTCHA v3</b>, and add your store domain (e.g., <code>{selectedStore?.custom_domain || `${selectedStore?.subdomain}.${getRootDomain()}`}</code> or <code>{selectedStore?.subdomain}.{getRootDomain()}</code>) <b>without <code>https://</code> at the beginning and without any paths or slashes at the end like <code>/mc</code> (enter the clean domain host only)</b>.</li>
